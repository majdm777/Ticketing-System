...
import { type RequestGroupView } from './request-row';
    ...
    <div className="space-y-6">
      ...
      {event ? (
        <>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:pb-0">
                  {statusOptions.map((status) => (
                    <Link
                      key={status}
                      href={`/admin/bookings?eventId=${eventId}&status=${status}`}
                      className={`inline-flex h-11 items-center whitespace-nowrap rounded-md border px-3 text-sm ${
          ...
          <BookingList requestGroups={requestGroups} showScroll={showScroll} />
